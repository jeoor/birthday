...
export function MinimalCake({ buttonRef, candleState, onBlow }: MinimalCakeProps) {
  ...
  const showsLitCake =
    ...
    candleState === 'lit' ||
    candleState === 'wavering'
  const imageSrc = `${import.meta.env.BASE_URL}images/${showsLitCake ? 'birthday-cake-lit-final' : 'birthday-cake-extinguished-final'}.webp`
  ...
  return (
    <motion.button
      ...
      aria-label={canBlow ? '点击蛋糕吹灭蜡烛' : `${BIRTHDAY.name} 的生日蛋糕`}
      ...
      <span className="birthday-cake-frame">
        {imageFailed ? (
          ...
          <>
            ...
            <CandleFlames state={candleState} />
          </>
        )}
      </span>
    </motion.button>
